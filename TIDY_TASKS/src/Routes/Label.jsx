...
function Label() {
    const { view, resMessage, deleteNote, isExpanded, setIsExpanded, getAll, dataUpdate } = useAuthStore();
    ...
    const filtered = data.filter(e => e.labels.includes(label)) || [];
            ...
            {resMessage && (
                <div className={`overflow-auto flex-1 min-h-0 p-5 md:p-7 ${isExpanded ? "opacity-40 pointer-events-none" : ""}`}>
                    ...
                    <div className="flex items-center gap-2 mb-6">
                        ...
                        <h1 className="text-sm font-semibold text-foreground tracking-tight capitalize">{label}</h1>
            ...
            {!resMessage && (
                <div className="flex-1 flex flex-col justify-center items-center gap-5 px-6 text-center">
                    <div className="w-16 h-16 rounded-2xl bg-muted flex items-center justify-center">
                        <MdLabelOutline className="size-8 text-muted-foreground/60" />
                    </div>
                    <div>
                        <h2 className="text-xl font-semibold text-foreground">No notes in this label</h2>
                        <p className="text-sm text-muted-foreground mt-1 max-w-xs">
                            Notes tagged with &quot;{label}&quot; will appear here.
                        </p>
                    </div>
                </div>
            )}
